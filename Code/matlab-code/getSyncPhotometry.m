...
function syncPhotometry = getSyncPhotometry(photometryData, intanData)
    ...
    idx = photometryData.syncpulseOnOff;
    M = numel(photometryData.highLow);
    barcodePhot = zeros(M+1,1); %High/low signal
    barcodePhot(idx(:,1)) = barcodePhot(idx(:,1)) + 1;
    barcodePhot(idx(:,2)+1) = barcodePhot(idx(:,2)+1) -1;
    barcodePhot = cumsum(barcodePhot);
    barcodePhot = barcodePhot(1:M) > 0;
    ...
    photTimestampsSr = (maxLag:length(barcodePhotInterpolated) + maxLag-1)'; % shifted timestamps
