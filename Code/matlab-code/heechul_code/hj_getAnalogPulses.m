...
function [pulses] = hj_getAnalogPulses(varargin)
...
if isempty(d) && isempty(filename)                                         % is d is not a signal, and not filename specified
    ...
    if isempty(f) || f.bytes == 0                                          % if analogin is empty or doesn't exist
        ...
        num_channels = length(aux_input_channels); % ADC input info from header file
        ...
        % Handle 0-based indexing (Intan) vs 1-based indexing (Matlab)
        % Assuming input analogCh is 0-based (consistent with bz_LoadBinary branch)
        if max(analogCh) < num_channels
             % If user passed e.g. 7 for 8 channels, it's likely 0-based.
             % If user passed 8 for 8 channels, it would be out of bounds if we add 1, 
             % but valid if we don't.
             % However, bz_LoadBinary branch uses analogCh+1, so we should be consistent.
             d = v(analogCh+1,:);
        else
             % Fallback if indices seem to be 1-based already
             d = v(analogCh,:);
        end
...
for jj = 1 : size(d,1)
    ...
    pul{jj} = zeros(2,0);
    cnt = 0;
    for ii = 1 : length(locsA)
        t_start = locsA(ii);
        possible_ends = locsB(locsB > t_start);
        if ~isempty(possible_ends)
            % Take the first end after start
            t_end = possible_ends(1);
            cnt = cnt + 1;
            pul{jj}(1,cnt) = t_start;
            pul{jj}(2,cnt) = t_end;
        end
    end
    ...
    if ~isempty(pul{jj})
        ...
        fprintf('    Found %d pulses on Channel %d.\n', size(pul{jj}, 2), analogCh(jj));
    else
        dur{jj} = [];
        stimPer{jj} = [];
        fprintf('    No pulses found on Channel %d.\n', analogCh(jj));
    end
    
    if ~isempty(analogCh)
        eventID{jj} = ones(size(dur{jj})) * analogCh(jj);
    else
...
filetarget = split(pwd,filesep); filetarget = filetarget{end};
hasPulses = exist('pul','var') && any(~cellfun(@isempty, pul));

if hasPulses % if no pulses, not save anything... 
    pulses.timestamps = cell2mat(pul)';
    pulses.amplitude = cell2mat(val)';
    pulses.duration = cell2mat(dur)';
    pulses.intsPeriods = vertcat(stimPer{:});
