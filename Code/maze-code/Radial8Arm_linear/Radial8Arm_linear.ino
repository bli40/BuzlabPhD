...
void setup() {
  ...
  delay(1000);
  //Serial.print("\n\n");
  Serial.println("000//mRAM//linear");
...
void loop() {
  ...
  for (int i = 0; i < 8; i += 1) {
    if (digitalRead(pinsIR[i]) == HIGH & lastIR[i] == 0) {
      ...
      for (int a = 0; a < nCued; a+=1) {
        ...
        else if (pinsIR[cued[a]] != pinsIR[i] && pinsIR[home] != pinsIR[i]){
          bonus = false;
        }
      }
  ...
  // checks duration after trial end to initiate next trial.
