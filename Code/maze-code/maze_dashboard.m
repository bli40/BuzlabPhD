%% Dashboard Script
% simple script to run in a loop and update a plotting window whenever a
% particular set of new lines come into the serial port from the maze
% arduino.
clear all;
close all;
...
%% Variables
rodent = 'M011';
thisDay = string(datetime('now','format','yyyy-MM-dd--HH-mm-ss'));

numbufferlines = 100000;
logStrings = cell(numbufferlines,1);
timestamps = nan(numbufferlines,1);
codelog = cell(numbufferlines,1);
pokelog = nan(numbufferlines,1);
displog = nan(numbufferlines,1);
cued = [];
rewardRate = [];
trialRewardRate = [];
windowRewardRate = [];
...
redbox = annotation('rectangle', [0 0 1 1], ...
    'Color', 'red', ...
    'LineWidth', 10, ...
    'Units', 'normalized', ...
    'FaceColor','red', ...
...
textITI = text(-12,3,0,'ITI:',"FontSize",15,"Color",col(2,:));
sessionDuration = text(-12,-3.5,0,sprintf('Session\nDuration:'),'FontSize',20,'Color',col(7,:));
% timeInTrials = text(-12,-5.5,0,sprintf('Time-In-Trials: '),'FontSize',18,'Color',col(4,:));
...
ylabel('Reward Rate');
xlabel('Trial Number');
title(sprintf('%s %s',thisDay,rodent),'FontSize',20);
%% trial durations (5)
...
device = serialport(port, baud, "Timeout", 60*60);
configureTerminator(device, "LF");   % Arduino sends \n at end of each line

butt = uifigure('Name', 'Serial Controller', ...
    'Position', [800, 500, 300, 200]);
btn = uibutton(butt, ...
    ...
    'FontSize', 16, ...
    'FontWeight', 'bold', ...
    'ButtonPushedFcn', @(~, ~) playButtonCallback(device, butt));
butt.Visible = 'off';

disp("Reading from " + port + " -- press Ctrl+C to stop");

for i = 1:100000
    line = readline(device);
    dt = datetime("now","Format","HH:mm:ss.SSSS");
    fprintf("[%s] %s",dt,line);
    
    lineElements = split(line{1}(1:end-1),'//');
    logStrings{i} = sprintf("[%s] %s",dt,string(line{1}(1:end-1)));

    now = datetime('now','format','yyyy-MM-dd--HH-mm-ss');
    sessionDuration.String = sprintf('Session\nDuration: %s',(now - thisDay));
...
%% Save Data
data2save = logStrings(~cellfun('isempty',logStrings));
% now = datetime('now','format','yyyy-MM-dd--HH-mm-ss');
logname = fullfile('B:','Brian','Training', ...
    join([string(now), ...
    '-', ...
    rodent],''));
writecell(data2save,logname);

name = fullfile('B:','Brian','Training', ...
    join([string(now), ...
    '-', ...
    rodent, ...
    '.jpg'],''));
...
function updateDiagram(line, cued, arm, varText)
    ...
    nexttile(1);
    if strcmp(v2,'mRAM')
        text(-12,7,0,upper(v3),'FontSize',30,'FontWeight','bold','VerticalAlignment','baseline');
...
function playButtonCallback(device, fig)
    % 4. Perform serial write
    % Sends an ASCII string and terminator (CR/LF). 
    writeline(device, ' '); 
    
    % 5. Hide the figure box
    fig.Visible = 'off';
end

%% Poisson Generator Test Code
close all;
log = logStrings(~cellfun('isempty',logStrings));
log = cellfun(@(x) double(extract(x, strlength(x))), log);
itis = (5*log) + 10;
histogram(itis);