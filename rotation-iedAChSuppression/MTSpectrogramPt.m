function [spectrogram,f] = MTSpectrogramPt(spikeTrain,varargin)
...
if size(spikeTrain,2) ~= 1 && size(spikeTrain,2) ~= 2
...
if isempty(frequency)
	if size(spikeTrain,2) == 2
		frequency = 1/median(diff(spikeTrain(:,1)));
	else
		frequency = f;
	end
end
...
% if size(spikeTrain,2) == 1
%     spikeTrain(:,2) = spikeTrain(:,1);
%     spikeTrain(:,1) = [1:size(spikeTrain,1)]./frequency;
% end
...
parameters.pad = pad;
[spectrogram,t,f,r] = mtspecgrampt(spikeTrain,[window window-overlap],parameters);
f = f';
